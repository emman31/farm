 ...
 */
function Action(obj, actionCallback, args, conditionActions, timeout) {
  this._obj = obj;
...
//  }
//  else {
//    return this._conditionCallback.Execute();// this._obj[this._conditionCallback](this._args, fallbackAction);
//  }
//};
...
Action.prototype.Execute = function Execute() {
  ...
  if (this._conditionActions && this._conditionActions.constructor === Array) {
    for (var i = 0; i < this._conditionActions.length; i ++) {
      this._conditionActions[i].Execute();
    }
  }
  ...
  return this._obj[this._actionCallback](this._args);
};
